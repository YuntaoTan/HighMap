...
static void dump_haplotype_matrix(LocsINFO *locsINFO);
...
LocsINFO *read_loc_pwd_info(MapOpt *opt){
	...
	/***************** DEBUG **************************
	dump_haplotype_matrix(locsINFO);
    ***************************************************/
...
void map_building_spatial_sample(LocsINFO *locsINFO, TSPMSET *tspm, MapOpt *opt){
	...
	for (sample_stage = 0;sample_stage < 6; sample_stage++ ){
		// if all loci mapped, then quit 
		if (tspm->Tlen == 0) break;
		// spatial sampling 
		...
		if (isempty == 0) continue;  // empty sample at this threshold of rec.freq
...
TSPMSET *init_tspm(LocsINFO *locsINFO, MapOpt *opt){
	...
	for (i=0;i<nloc ;i++ ){
		tspm->T[i] = i; 
		tspm->S[i] = i; tspm->SD[i] = -1;
		tspm->P[i] = i; tspm->PD[i] = -1;
		tspm->M[i] = i; tspm->MD[i] = -1;
	}
	// Tlen, Slen, Plen, Mlen 
	tspm->Tlen = nloc;
	tspm->Slen = 0;
...
static void dump_haplotype_matrix(LocsINFO *locsINFO){
	...
	for (i=0; i<locsINFO->nloc; i++){
		printf("%s\t<%s>\t{%s}", locsINFO->gMatrix[i]->marker, locsINFO->gMatrix[i]->type, locsINFO->gMatrix[i]->phase);
		if (strcmp(locsINFO->gMatrix[i]->type,"abxcd") == 0 || strcmp(locsINFO->gMatrix[i]->type, "efxeg") == 0){
			for (j=0; j<locsINFO->nind; j++){
				printf("\t%d%d", locsINFO->hapMatrix[i][j]->hapP, locsINFO->hapMatrix[i][j]->hapM);
			}
		}else if (strcmp(locsINFO->gMatrix[i]->type, "hkxhk") == 0){
			for (j=0; j<locsINFO->nind; j++){
				if (strcmp(locsINFO->gMatrix[i]->genotype[j],"hk") == 0){
					printf("\t%s%s", "x", "x");
				}else{
					printf("\t%d%d", locsINFO->hapMatrix[i][j]->hapP, locsINFO->hapMatrix[i][j]->hapM);
				}
		...
		}else if (strcmp(locsINFO->gMatrix[i]->type, "lmxll") == 0){
			for (j=0; j<locsINFO->nind; j++){
				printf("\t%d-", locsINFO->hapMatrix[i][j]->hapP);
			}
		}else{
			for (j=0; j<locsINFO->nind; j++){
				printf("\t-%d", locsINFO->hapMatrix[i][j]->hapM);
			}
		}
		printf("\n");
	}
